...
> = {
  sm: { minH: 'min-h-[260px]', pad: 'px-6 pt-5', footer: 'px-6 py-3', title: 'font-display text-m sm:text-l font-bold uppercase', headline: 'text-[46px]' },
  md: { minH: 'min-h-[380px]', pad: 'px-8 pt-7', footer: 'px-8 py-4', title: 'font-display text-l sm:text-xl font-bold uppercase', headline: 'text-[72px]' },
  lg: { minH: 'min-h-[460px]', pad: 'px-10 pt-9', footer: 'px-10 py-5', title: 'font-display text-xl sm:text-2xl font-bold uppercase', headline: 'text-[88px]' },
};
...
export default function ProgressMetricCard({
  ...
  const sz = SIZES[size];
  const shell = `relative flex ${sz.minH} w-full flex-col overflow-hidden rounded-[28px] border border-white/12 bg-[#080c14]/75 backdrop-blur-xl shadow-[0_15px_35px_-10px_rgba(0,0,0,0.7)] transition-all duration-500 hover:border-[#10b981]/60 hover:shadow-[0_0_35px_rgba(16,185,129,0.22)] ${className}`;
  ...
  if (loading) {
    return (
      <div className={shell} aria-busy="true">
        ...
        <div className={`border-t border-white/10 ${sz.footer}`}>
  ...
  if (!hasData) {
    return (
      <div className={shell}>
        <div className={`flex flex-1 flex-col ${sz.pad}`}>
          <h2 className={`${sz.title} font-semibold tracking-tight text-foreground`}>{title}</h2>
  ...
  return (
    <div className={shell}>
      ...
      <div className="absolute inset-y-0 right-0 z-0 pl-1 sm:pl-1" style={{ width: `${REGION_W}%` }}>
        <div
          className="absolute inset-0"
          style={{ background: `linear-gradient(to left, ${color.stroke}1f, transparent 75%)` }}
        />
        <div
          className="absolute inset-0 text-white/[0.08]"
          style={{
      ...
      <div className={`pointer-events-none relative z-10 flex flex-1 flex-col ${sz.pad}`}>
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0 max-w-[280px] sm:max-w-[340px]">
            <h2 className={`${sz.title} tracking-tight text-foreground leading-snug`}>{title}</h2>
          </div>
          <span className="shrink-0 whitespace-nowrap glass-pill px-3 py-1.5 rounded-full text-xs font-mono text-[#00f0ff] border border-white/15 font-bold tracking-wider uppercase mt-1">
            40+ Components Made
          </span>
        </div>
        ...
        <div
          className={`mt-8 sm:mt-10 ${sz.headline} font-medium leading-none tracking-tight text-foreground`}
        >
          {displayTotal}
        </div>
      </div>
      ...
      <div
        className={`relative z-10 flex items-center justify-between gap-4 border-t border-white/10 bg-black/40 ${sz.footer} text-[14px]`}
      >
        <div>
